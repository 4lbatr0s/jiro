...
import { format } from 'date-fns'
  //INFO: How to use an awaited type in typescript.
        ...
        <Card>
          ...
          <CardFooter className="flex flex-col items-start space-y-2 md:flex-row md:justify-between md:space-x-0">
            <Button type="submit">
                ...
                {subscriptionPlan.isSubscribed ? "Manage Subscription" : "Upgrage to PRO"}
            </Button>
            {subscriptionPlan.isSubscribed ? (
                <p className="rounded-full text-xs font-medium">
                    {
                        subscriptionPlan.isCanceled ? "Your plan will be cancelled on "
                        : "Your plan renews on "
                    }
                    {format(
