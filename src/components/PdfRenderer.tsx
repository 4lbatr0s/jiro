...
const PdfRenderer = ({ url }: PdfRendererProps) => {
  ...
  const CustomPageValidator = z.object({
    //num => user input
    //REFINE: consider valid only if these two requirements are satisfied.
    page: z
      .string()
  ...
  //to handle page change through user input
  ...
  const setCurrentPageNext = () => {
    setCurrentPage((prev) => {
      const nextPage = prev + 1 > numPages! ? numPages! : prev + 1;
  ...
  const setCurrentPagePrevious = () => {
    setCurrentPage((prev) => {
      const prevPage = prev - 1 > 1 ? prev - 1 : 1;
      setValue("page", String(prevPage));
      return prevPage;
    });
  };

  const { toast } = useToast();
  //INFO: ref, refers to the
